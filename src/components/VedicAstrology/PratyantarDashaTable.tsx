...
interface PratyantarDashaTableProps {
...
const PratyantarDashaTable: React.FC<PratyantarDashaTableProps> = ({ 
  pratyantarDashas, 
  currentPlanet 
}) => {
  ...
  return (
    <Card className="mt-3 border-l-4 border-amber-500">
      <CardHeader className="py-2 px-3">
        <CardTitle className="text-sm">
          Phụ Tiểu Chu Kỳ (Pratyantardasha) - {getViPlanetName(currentPlanet)}
        </CardTitle>
      </CardHeader>
      <CardContent className="py-2 px-3">
        <Table>
          ...
          <TableBody>
            {pratyantarDashas.map((dasha, index) => (
              <TableRow key={index}>
...
export default PratyantarDashaTable;
