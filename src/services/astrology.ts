...
// Set ephemeris path for browser environment
try {
  swisseph.swe_set_ephe_path('/ephe');
} catch (error) {
  console.warn('Browser environment, skipping ephemeris path setup');
}
...
export async function calculateBirthChart(date: Date, lat: number, lon: number): Promise<BirthChart> {
  try {
    ...
    // Calculate planets with Moshier ephemeris
    const planets = await Promise.all(
      Object.keys(planetIds).map(async (name) => {
        const id = planetIds[name];
        const result = await new Promise((resolve, reject) => {
          swisseph.swe_calc_ut(jd, id, swisseph.SEFLG_SIDEREAL | swisseph.SEFLG_MOSEPH | swisseph.SEFLG_SPEED, (res: any) => {
    ...
    // Calculate houses with Moshier method
