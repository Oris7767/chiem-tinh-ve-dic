...
// Direct PNG download (simplified version - just chart)
export async function downloadAsPNGDirect(
  ...
  try {
    ...
    const fullCanvas = document.createElement('canvas');
    fullCanvas.width = 1190;
    fullCanvas.height = 1684;
    const ctx = fullCanvas.getContext('2d');
    ...
    ctx.font = 'bold 36px Arial';
    ctx.textAlign = 'center';
    ctx.fillText(`Vedic Birth Chart${userData?.name ? ` - ${userData.name}` : ''}`, 595, 55);

    // Draw chart centered
    ctx.drawImage(svgCanvas, 345, 140, 500, 500);

    // Footer
    ctx.fillStyle = '#666';
    ctx.font = '18px Arial';
    ctx.fillText(`Generated by Vedic Astrology App - ${new Date().toLocaleDateString('vi-VN')}`, 595, 1654);
