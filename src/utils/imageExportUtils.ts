...
export async function downloadAsPDF(
  ...
  try {
    ...
    // Scale context for high resolution (text will be sharp)
    ctx.scale(pdfScale, pdfScale);

    // Draw all content on the scaled canvas...
    ...
    // Calculate what dimensions to use in PDF to maintain aspect ratio
    // The content was drawn at contentWidth x contentHeight logical pixels
    // So we scale that to fit the PDF page
    const aspectRatio1 = contentWidth / contentHeight;
    const pageAspect1 = pageWidth / pageHeight;

    let pdfW1 = pageWidth;
    let pdfH1 = pageHeight;

    // If aspect ratios don't match, fit to the dimension that limits more
    if (aspectRatio1 > pageAspect1) {
      pdfH1 = pageWidth / aspectRatio1;
    } else {
      pdfW1 = pageHeight * aspectRatio1;
    }

    // Center the image on the page
    const imgX1 = (pageWidth - pdfW1) / 2;
    const imgY1 = (pageHeight - pdfH1) / 2;

    pdf.addImage(imgData1, 'PNG', imgX1, imgY1, pdfW1, pdfH1);
    pdf.addPage();
    ...
    // Same aspect ratio fitting for page 2
    const aspectRatio2 = contentWidth / contentHeight;
    const pageAspect2 = pageWidth / pageHeight;

    let pdfW2 = pageWidth;
    let pdfH2 = pageHeight;

    if (aspectRatio2 > pageAspect2) {
      pdfH2 = pageWidth / aspectRatio2;
    } else {
      pdfW2 = pageHeight * aspectRatio2;
    }

    const imgX2 = (pageWidth - pdfW2) / 2;
    const imgY2 = (pageHeight - pdfH2) / 2;

    pdf.addImage(imgData2, 'PNG', imgX2, imgY2, pdfW2, pdfH2);
